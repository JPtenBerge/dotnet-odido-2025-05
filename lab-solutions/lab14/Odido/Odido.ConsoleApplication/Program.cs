...
void Lab14()
{
   ...
   // - With a list of `int` values(print whether each number is even or odd)
    ...
    {
        new CombatLogEntry("Hero1 attacked Boss1"),
        new CombatLogEntry("Boss1 attacked Hero1")
    };

    List<int> numbers = new List<int> { 1, 2, 3, 4, 5 };

    // Print character details
    Utilities.ForEach(characters, character => Console.WriteLine($"Type: {character.GetType().Name}, Name: {character.Name}, Health: {character.Health}"));

    // print each combat log entry
    Utilities.ForEach(combatLogEntries, entry => Console.WriteLine($"{entry.Timestamp.ToShortTimeString()} {entry.Message}"));

    //print whether each number is even or odd
    Utilities.ForEach(numbers, number => Console.WriteLine($"{number} is {(number % 2 == 0 ? "even" : "odd")}"));
}