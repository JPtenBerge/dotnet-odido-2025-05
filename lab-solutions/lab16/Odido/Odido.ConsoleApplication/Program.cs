...
async Task CreateGame() {
    game = await service.CreateNewGame(playerName);
...
async Task PlayTurn()
{
    ...
    game = await service.PlayTurn(game.Id, action);
    DisplayGame();
}
...
async Task DisplayGames()
{
    var games = await service.GetGamesByPlayer(playerName);
    if (games.Count == 0)
    {
...
async Task SelectGame() {
    ...
    game = await service.GetGameById(gameId);
    if (game is null)
    {
